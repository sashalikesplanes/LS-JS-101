const searchWord = (searchWord, text) =>
  text
    .split(" ")
    .map((word) => {
      let tempWord = word.toLowerCase().match(/[a-z]+/g)[0]; // remove punctuation surrounding word
      if (tempWord === searchWord) return `**${word.toUpperCase()}**`;
      else return word;
    })
    .join(" ");
